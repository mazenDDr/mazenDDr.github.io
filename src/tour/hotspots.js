...
import { B, boxB, hitBox } from './m.js';
import { Doodles } from './doodles.js';

// Labels for the places (what the back button and the markers are called).
export const LABELS = { couch: 'Sit on the couch', tv: 'Watch TV', games: 'Play a game', desk: 'Sit at the desk', pc: 'Use the computer',
  bed: 'Sit on the bed', memo: 'Read the pinboard', certificates: 'Look at the diploma', room: 'Stand in the room' };
...
export class Hotspots {
  constructor(camera, director, onAction = () => {}, root = document.body) {
    ...
    this.spots = SPOTS.map((s) => {
      const box = boxB(...s.box);
      const el = document.createElement('button');
      el.className = 'spot';
      s.label ||= LABELS[s.place];
  ...
  pick(e) {
    ...
    const ray = this.camera.ray((e.clientX / innerWidth) * 2 - 1, -(e.clientY / innerHeight) * 2 + 1);
    let best = null, bestD = Infinity;
    for (const s of this.visibleSpots()) {
      const d = hitBox(ray, s.box);
  ...
  setHover(s) {
    if (s !== this.hover) this.onHover?.(s);
    this.hover = s;
