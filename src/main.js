...
const roomReady = loadRoom(renderer, (p) => { progress = p; bar.style.width = `${p * 100}%`; }).then(({ room, parts }) => {
  scene.add(room);
...
window.__room = { director, spots, camera, ready: false };
