...
a light switch, outlets and cables.
...
| web | 0.271 | 0.610 | 2.253 | 0.049 | 0.521 | 0.473 | 0.367 |

## Motion

Walking is a gimbal glide (`src/director.js`): level horizon, steady cruise with
soft 0.6 s ramps, the view turning a little ahead of the path on a damped spring
capped at 110°/s, and sitting only in the last 0.9 m. Measured with
`tools/motion_probe.py` on the same route, against the previous head-bob walk:

| | before | now |
|---|---:|---:|
| vertical reversals per second (bob) | 2.64 | 0.55 |
| acceleration, 99th percentile | 7.1 m/s² | 4.1–4.8 m/s² |
| turn rate, 99th percentile | 532 °/s | 121 °/s |
| turn acceleration, 99th percentile | 8,261 °/s² | 1,500 °/s² |
